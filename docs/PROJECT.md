...
| ДДС (движение денег) | Отчёт «движение денежных средств» по месяцам и статьям ДДС: приток/отток и чистый поток за период (только согласованные операции). Данные из движений денег (`/api/money/dds`) |
