...
| Раздел | Что делает |
|---|---|
| Сводка дня (дашборд) | Экран руководителя «как идут дела сегодня»: касса по филиалам и расхождения с iiko (недостача/излишек), расходы на согласовании, % чек-листов, алерты, а также блок «План месяца» (% выполнения плана выручки и темп опережения/отставания к сегодня из `/api/plan`). Собирает уже имеющиеся данные (`/api/dashboard`), управляющий видит свой филиал |
