...
test("журнал безопасности: директору можно, staff — 403, события читаются", async () => {
  // Директорская учётка из этого файла уже логинилась — событие login есть.
  ...
  const list = await fetch(`${base}/api/audit?event=login&limit=50`, {
    headers: { Authorization: `Bearer ${dir.token}` },
  });
  assert.equal(list.status, 200);
  const { items } = await list.json();
  assert.ok(items.length >= 1);
  assert.ok(items[0].at && items[0].event === "login");

  // Линейному персоналу журнал не положен: SSO-учётка имеет роль staff.
  ...
  const denied = await fetch(`${base}/api/audit`, {
