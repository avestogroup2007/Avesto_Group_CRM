...
r.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Выгрузка всего реестра денег — фиксируем в журнале безопасности (кто и
    // с каким фильтром выгрузил), как и экспорт резервной копии.
    await db.auditLog
      .create({
        data: {
          userId: req.user.uid,
          event: "money_export",
          detail: `Экспорт реестра денег в CSV: строк ${items.length}, период ${from || "…"}–${to || "…"}${branch ? `, филиал ${branch}` : ""}`,
          ip: req.ip,
        },
      })
      .catch(() => {});
